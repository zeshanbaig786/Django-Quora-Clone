...
def Question_update(request, id):
    ...
    if request.user == instance.user.user:
        top = Profile.objects.all().order_by("-points")[
            : min(Profile.objects.count(), 5)
        ]
        if request.user.is_authenticated:
            profile = get_object_or_404(Profile, user=request.user)
            if profile.points < 100:
...
def Question_detail(request, id):
    ...
    queryset_list = (
        ...
        .order_by("-accepted", "-votes", "-timestamp")
    )
    query = request.GET.get("q")
    if query:
        queryset_list = queryset_list.filter(
            Q(answer__icontains=query) | Q(user__user__username__icontains=query)
        ).distinct()
...
def User_list(request):
    ...
    if query:
        queryset_list = queryset_list.filter(
            Q(location__icontains=query) | Q(user__username__icontains=query)
        ).distinct()
    paginator = Paginator(queryset_list, 10)
    page = request.GET.get("page")
    username = "Login"
